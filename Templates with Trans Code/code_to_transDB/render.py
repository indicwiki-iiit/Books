...
###################################################################
def apostrophe_remover(inp):
    new_inp = ""
    i = 0
    while i <len(inp):
        try:
            if inp[i].isalpha() and inp[i+1]=="'":
                if inp[i+2]==']':
                    new_inp += inp[i] + inp[i+1]
                elif inp[i+2]=="," and inp[i+3]==" " and inp[i+4]=='"':
                    new_inp += inp[i] + inp[i+1]
                elif inp[i+2]=="," and inp[i+3]=='"':
                    new_inp += inp[i] + inp[i+1]
                elif inp[i+2]=="," and inp[i+3]==" " and inp[i+4]=="'":
                    new_inp += inp[i] + inp[i+1]
                elif inp[i+2]=="," and inp[i+3]=="'":
...
def pro_moderator(inp,mode='unknown'):
    if inp=='':
        ...
        if mode=='author1_image' or mode=='author1_education':
            if inp[0]=='[':
                if mode=='author1_education':
                    inp = inp.replace("'s","s").replace("'t","t").replace("\n","")
                    inp = apostrophe_remover(inp)
            ...
            if inp[0]=='[':
                ...
                if mode=='author1_awards_received':
                    inp = inp.replace("'s","s")
                    inp = apostrophe_remover(inp)
                    inp = '[\''+inp+'\']'
                    return ast.literal_eval(inp)
...
def getInfo(row): 
    ...
    # Dimensions
    if row['amazon_book_specifications']!='':
    ...
    data['awards_as_category'] = row['awards_as_category']
    ...
    for i in pro_moderated_attributes:
        ...
        elif i=='author1_awards_received':
            x = pro_moderator(row[i],'author1_awards_received')
        else:
    ...
    if data['author1_notable_works']!='':
        temp = []
        for i in data['author1_notable_works']:
            if i!=data['book_title']:
                temp.append(i)
        if len(data['author1_notable_works'])>5:
            data['author1_notable_works'] = temp[:5]
        else:
            data['author1_notable_works'] = temp
    else:
        data['author1_notable_works']=''

    if data['publisher_collection']!='':
        temp = []
        for i in data['publisher_collection']:
            if i!=data['book_title']:
                temp.append(i)
        if len(data['publisher_collection'])>5:
            data['publisher_collection'] = temp[:5]
        else:
            data['publisher_collection'] = temp
    else:
        data['publisher_collection']=''
    
    if data['more_by_author']!='':
        temp = []
        for i in data['more_by_author']:
            if i!=data['book_title']:
                temp.append(i)
        if len(data['more_by_author'])>5:
            data['more_by_author'] = temp[:5]
        else:
            data['more_by_author'] = temp
    else:
        data['more_by_author']=''
    ...
    if row['goodreads_link'] != "":
        ...
        data['GR'] = GR
        data['goodreads_link'] = row['goodreads_link']
    else:
        data['GR'] = ''
        data['goodreads_link'] = ''
    ...
    if row['librarything_url'] != "":
        ...
        data['LT'] = LT
        data['librarything_url'] = row['librarything_url']
    else:
        data['LT']= ''
        data['librarything_url'] = ''
    ...
    if row['author1_wiki'] != "":
        AU = row['author1_wiki']
        data['AU'] = AU
        data['author1_wiki'] = row['author1_wiki']
    else:
        data['AU'] = ''
        data['author1_wiki'] = ''
    ...
    cat_l = data['language']
    cat_a = data['awards_as_category']
    

    data['CATEGORY_YEAR']  = '[[Category: ' + cat_y + ' పుస్తకాలు'+']]'

    # The following two are not required here actually, because their associated attributes are transliterated in trans_render.py
    # data['CATEGORY_GENRE'] = '[[Category: ' + cat_g + ' రచనాశైలిలో రచింపబడిన పుస్తకాలు'+']]'
    # data['CATEGORY_LANGUAGE'] = '[[Category: ' + cat_l + ' పుస్తకాలు'+']]'
    ...
    if cat_a=='':
        ...
        awards_lis = []
        for i in data['awards_as_category']:
            awards_lis.append('[[Category: ' + i + ' పొందిన పుస్తకాలు'+']]')

        data['CATEGORY_AWARDS'] = awards_lis
