...
###################################################################
# SKIP TO LINE 51 TO SEE THE CODE FROM getData()
# SKIP TO LINE 139 TO SEE THE MAIN CODE WHICH RENDERS THE TEMPLATE
...
def clean_published_date(row):
    # Published Dates collected from Amazon in the attribute 'amazon_book_specifications' are very neat with the perfect MM-DD-YYYY format.
    # But dates from this attribute have 116 missing values, while the ones from the 'published_date' attribute have 0 null values, though in inconsistent date formats.
    # So, here, values are primarily taken from 'amazon_book_specifications'. If missing, values are taken from the 'published_date' attribute, but only YYYY.
    # Length is checked in the template. If YYYY is sent from these 116 values, length is == 4 and a different sentence should be printed.
    
    pub = ''
    
    if 'Publication date' in row['amazon_book_specifications'][list(row['amazon_book_specifications'].keys())[0]]:
        pub = row['amazon_book_specifications'][list(row['amazon_book_specifications'].keys())[0]]['Publication date']
    
    if pub=='':
        # If the date from 'amazon_book_specifications' is missing
        # Returning only YYYY
        if str(row['isbn_13'])=='9780310257868':
            pub = '2007'
        else:
            pub = row['published_date'][0:4]
    return pub

    

def date_for_category(inp):
    if len(inp)==4:
        return inp
    else:
        x = len(inp)
...
# THE getData() METHOD AND THE MAIN CODE ARE BELOW THIS LINE
###################################################################
...
def getData(row): 
    ...
    data['published_date'] = clean_published_date(row)
    ...
    GR_WIKI_LINK = "<ref name='gr-link'>{{{{cite web |url={} |title={} - గుడ్ రీడ్స్}}}}</ref>".format(GR,row['book_title'])
    LT_WIKI_LINK = "<ref name='lt-link'>{{{{cite web |url={} |title={} - లైబ్రరీ థింగ్}}}}</ref>".format(LT,row['book_title'])
    GG_WIKI_LINK = "<ref name='gg-link'>{{{{cite web |url={} |title={} - గూగుల్ బుక్స్}}}}</ref>".format(GG,row['book_title'])
    AM_WIKI_LINK = "<ref name='am-link'>{{{{cite web |url={} |title={} - అమెజాన్}}}}</ref>".format(AM,row['book_title'])
    WC_WIKI_LINK = "<ref name='wc-link'>{{{{cite web |url={} |title={} - వరల్డ్ కాట్}}}}</ref>".format(WC,row['book_title'])
    AU_WIKI_LINK = "<ref name='wc-link'>{{{{cite web |url={} |title={} - వికీపీడియా}}}}</ref>".format(AU,row['author1_name'])

    data['REF_GR'] = GR_WIKI_LINK
    data['REF_LT'] = LT_WIKI_LINK
    data['REF_GG'] = GG_WIKI_LINK
    data['REF_AM'] = AM_WIKI_LINK
    data['REF_WC'] = WC_WIKI_LINK
    data['REF_AU'] = AU_WIKI_LINK

    ######################################
    # CATEGORIES
    ######################################

    cat_y  = date_for_category(data['published_date'])
...
# THE MAIN CODE TO RENDER THE TEMPLATE IS BELOW THIS
###################################################################
